const { EmbedBuilder, Collection, PermissionsBitField, isStringSelectMenu } = require('discord.js');
const ms = require('ms');
		...
		if(interaction.isStringSelectMenu()) {
			...
			try {
				if(selectMenu.permissions) {
					if(!interaction.memberPermissions.has(PermissionsBitField.resolve(selectMenu.permissions || []))) {
						const perms = new EmbedBuilder()
						.setDescription(`🚫 ${interaction.user}, You don't have \`${selectMenu.permissions}\` permissions to interact this button!`)
						.setColor('Red')
						return interaction.reply({ embeds: [perms], ephemeral: true })
					}
				}
				await selectMenu.run(client, interaction, args = []);
			} catch (error) {
				console.log(error);
			}
		}
